...
4. 실행:
   ...
   # 실제 마이그레이션
   bash ~/.claude/scripts/notion-migrate-worklogs.sh [--date YYYY-MM-DD] [--delete-after] <worklogs_dir>
   ```
...
6. **마이그레이션 완료 후 — 워크로그 저장 방식 변경 제안**:
   ...
   - 변경 선택 시:
     ```bash
     bash ~/.claude/scripts/notion-migrate-worklogs.sh --set-mode <mode> <worklogs_dir>
     ```
...
- 전체 성공한 파일만 삭제
- 워크로그 모드 변경은 `~/.claude/settings.json`의 `WORKLOG_DEST`, `WORKLOG_GIT_TRACK`을 업데이트
