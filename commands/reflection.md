...
- 개선 제안이 0건이면 승인 대기 없이 바로 Step 5로 진행한다.
- 제안이 있으면 사용자가 선택한 항목만 적용한다.
...
`/handoff` 스킬을 실행한다. (HANDOFF.md 작성 + Memory 갱신 + 커밋을 `/handoff`가 처리)

단, `/handoff`의 Memory 갱신과 커밋은 이미 Step 5, 7에서 처리하므로, `/handoff`에서는 HANDOFF.md 작성(Step 1)만 실행하고 나머지는 스킵한다.
