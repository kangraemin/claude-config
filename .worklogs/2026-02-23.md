...
### 작업 내용
- chore: 공식 문서 우선 원칙 추가 및 .claudeignore 삭제
- 비공식 .claudeignore 제거, CLAUDE.md에 공식 문서 확인 원칙 추가
...
### 작업 내용
- feat: 워크로그 소요 시간을 JSONL durationMs 합산으로 변경
- 벽시계 시간(wall clock) 대신 세션 JSONL의 durationMs를 합산하여
- 실제 Claude 작업 시간만 측정하도록 개선
...
### 작업 내용
- feat: 소요 시간 측정 헬퍼 스크립트 추가 (duration.py)
- JSONL durationMs 합산 로직을 독립 스크립트로 분리하여
- 워크로그에서 안정적으로 호출할 수 있도록 개선
