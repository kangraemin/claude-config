...
3. **Synthesis 체크**: 여러 주제를 가로지르는 상위 결론이 나왔나? 있으면 `library/synthesis/`에 종합 문서로 저장
4. **분류**: 각 항목의 유형(gotcha/strategy/pattern/decision)과 durability(permanent/temporal) 판단
5. **저장**: 바로 library에 파일로 작성 (확인 없이). 유형별 템플릿 사용
6. **CHANGELOG**: `~/.claude/.claude-library/CHANGELOG.md`에 추가한 항목 append
7. **커밋**: git commit & push
8. **보고**: 저장한 내용 한 줄 요약으로 알림
